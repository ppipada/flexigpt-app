...
// NewUserPromptsFTSListener returns a mapstore.FileListener that updates the FTS engine on file changes.
func NewUserPromptsFTSListener(e *ftsengine.Engine) mapstore.FileListener {
	return func(ev mapstore.FileEvent) {
		...
		if ev.File == "" ||
			...
			strings.HasSuffix(ev.File, spec.PromptBuiltInOverlayDBFileName) {
			return
		}
...
func processFTSSync(
	...
	if !strings.HasSuffix(fullPath, "."+bundleitemutils.ItemFileExtension) ||
		...
		strings.HasSuffix(fullPath, spec.PromptBuiltInOverlayDBFileName) {
